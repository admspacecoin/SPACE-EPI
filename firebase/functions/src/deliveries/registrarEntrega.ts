 ...
 */
export const registrarEntrega = onCall<Input>(async (request) => {
  ...
  if (!employeeId) throw new HttpsError('invalid-argument', 'employeeId é obrigatório.')
  if (!items || items.length === 0) {
    throw new HttpsError('invalid-argument', 'A entrega precisa ter pelo menos um item.')
  }
  for (const item of items) {
    if (!item.variantId || !Number.isInteger(item.quantidade) || item.quantidade <= 0 || !item.motivo) {
      throw new HttpsError('invalid-argument', 'Cada item precisa de variantId, quantidade (> 0) e motivo.')
    }
  }

  const employeeRef = db.collection('employees').doc(employeeId)
  const deliveryRef = db.collection('ppeDeliveries').doc()
  const inventoryRefs = items.map((i) => db.collection('inventory').doc(i.variantId))
  ...
  const deliveryId = await db.runTransaction(async (tx) => {
    ...
    items.forEach((item, idx) => {
      ...
      tx.set(
        inventoryRefs[idx],
        { obraId: employee.obraId, quantidadeAtual: novosSaldos[idx] },
        { merge: true }
      )
    })
